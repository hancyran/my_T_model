...
def getData_feat2(train_type):
    if train_type == 'cv':
        X_train = pd.read_hdf(pargs.tmp_data_path+'/tmp_train_feat2_ad.h5').values
...
def getData_feat3(train_type):
    if train_type == 'cv':
        X_train = np.load(pargs.tmp_data_path+'/tmp_train_feat3_withlen.npy')
        Y_train = np.load(pargs.train_data_path+'/train_arr_label.npy')
        test_df = pd.read_hdf(pargs.train_data_path+'/train_origin_final.h5')
        return X_train, Y_train, test_df
    elif train_type == 'val':
        ...
        X_test = np.load(pargs.tmp_data_path+'/tmp_test_feat3_withlen.npy')
