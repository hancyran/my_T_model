...
def getAdData(train_type):
    if train_type == 'cv':
        #### train dataset
        if os.path.exists(args.tmp_data_path + '/tmp_train_data.npy'):
            X_train = np.load(args.tmp_data_path + '/tmp_train_data.npy')
        else:
            X_train = np.load(args.train_data_path + '/train_arr_id.npy')
            for n, i in enumerate(args.ad_feat):
                if n == 0:
                    ...
                    X_train = np.concatenate((X_train, np.load(args.train_data_path + '/train_arr_%s_scaled.npy' % i)),
                                             axis=1)
            # # save as tmp file
            np.save(args.tmp_data_path + '/tmp_train_data.npy', X_train)
        ...
        if os.path.exists(args.tmp_data_path + '/tmp_tri_train_data.npy') \
                and os.path.exists(args.tmp_data_path + '/tmp_tri_train_label.npy') \
                and os.path.exists(args.tmp_data_path + '/tmp_val_train_data.npy') \
                and os.path.exists(args.tmp_data_path + '/tmp_val_train_label.npy'):
            X_tri_data = np.load(args.tmp_data_path + '/tmp_tri_train_data.npy')
            Y_tri_label = np.load(args.tmp_data_path + '/tmp_tri_train_label.npy')
            X_val_data = np.load(args.tmp_data_path + '/tmp_val_train_data.npy')
            Y_val_label = np.load(args.tmp_data_path + '/tmp_val_train_label.npy')
        ...
        ### test dataset
        if os.path.exists(args.tmp_data_path + '/tmp_test_data.npy'):
            X_test = np.load(args.tmp_data_path + '/tmp_test_data.npy')
        else:
            X_test = np.load(args.test_data_path + '/test_arr_id.npy')
            for n, i in enumerate(args.ad_feat):
                if n == 0:
                    pass
                elif n < 7:
                    ...
                    X_test = np.concatenate((X_test, np.load(args.test_data_path + '/test_arr_%s_scaled.npy' % i)),
                                            axis=1)

            np.save(args.tmp_data_path + '/tmp_test_data.npy', X_test)
